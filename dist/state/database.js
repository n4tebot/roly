...
export class StateDatabase {
    ...
    `);
        ...
        VALUES (?, ?, ?, ?)
      `).run(this.config.identity.agentId, null, // Genesis agent has no parent
            1, Date.now());
        }
    }
    /**
    ...
    `);
        stmt.run(turn.id, turn.timestamp.getTime(), turn.thought, turn.action?.tool || null, turn.action?.input ? JSON.stringify(turn.action.input) : null, turn.action?.output ? JSON.stringify(turn.action.output) : null, turn.action?.error || null, turn.observation, turn.reflection || null, null, // TODO: Add survival tier to turn
     ...
     * Store a metric
     */
    async storeMetric(name, value, metadata) {
        const stmt = this.db.prepare(`
     ...
     */
    async getMetrics(name, since, limit = 100) {
