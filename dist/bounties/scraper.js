...
export class BountyScraper {
    ...
    `;
        const db = this.database.db; // Access underlying SQLite database
        db.exec(sql);
    }
    /**
     * Scrape all bounties from all sources
     */
    async scrapeAllBounties() {
        const bounties = [];
        try {
    ...
    `);
        for (const bounty of bounties) {
            stmt.run(bounty.id, bounty.source, bounty.title, bounty.description, bounty.reward_amount, bounty.reward_token, bounty.deadline?.getTime() || null, bounty.url, JSON.stringify(bounty.skills), bounty.status, bounty.discovered_at.getTime(), bounty.claimed_at?.getTime() || null, bounty.metadata ? JSON.stringify(bounty.metadata) : null);
        }
     ...
     */
    async getBounties(status, source, limit = 50) {
        const db = this.database.db;
     ...
     */
    async updateBountyStatus(id, status, claimedAt) {
        const db = this.database.db;
        const stmt = db.prepare(`
      UPDATE bounties 
     ...
     */
    isValidSuperteamListing(listing) {
        return (listing &&
            listing.id &&
            listing.title &&
     ...
     */
    convertSuperteamToBounty(listing) {
        try {
    ...
    extractRewardFromGitHub(issue) {
        ...
        if (labels.includes('good-first-issue'))
            return 50 * 1_000_000; // $50
        if (labels.includes('help-wanted'))
            return 100 * 1_000_000; // $100
        if (labels.includes('bounty'))
            return 250 * 1_000_000; // $250
        if (labels.includes('bug-bounty'))
            return 500 * 1_000_000; // $500
        return 0; // Unknown reward
    }
    /**
     * Extract skills from GitHub issue
     */
    extractSkillsFromGitHub(issue) {
        ...
        for (const label of labels) {
            if (label.includes('rust'))
                skills.add('Rust');
            if (label.includes('typescript') || label.includes('ts'))
                skills.add('TypeScript');
            if (label.includes('javascript') || label.includes('js'))
                skills.add('JavaScript');
            if (label.includes('python'))
                skills.add('Python');
            if (label.includes('solana'))
                skills.add('Solana');
            if (label.includes('web3'))
                skills.add('Web3');
            if (label.includes('smart-contract'))
        ...
        if (text.includes('react'))
            skills.add('React');
        if (text.includes('solana') || text.includes('web3'))
            skills.add('Solana');
