...
export class BountyEvaluator {
    ...
    database;
    agentSkills;
    baseCostPerHour = 10_000_000; // $10/hour in micro-USDC
    constructor(config) {
        this.config = config;
     ...
     */
    async evaluateBounties(bounties) {
        const evaluations = [];
    ...
    async evaluateBounty(bounty) {
        ...
        const roi = bounty.reward_amount > 0
            ...
            : 0;
        const score = this.calculateOverallScore(roi, skillsMatch, urgency, confidence, difficulty);
        const reasoning = this.generateReasoning(bounty, difficulty, skillsMatch, roi, urgency);
     ...
     */
    estimateTimeRequired(bounty, difficulty) {
        const baseTimes = {
     ...
     */
    calculateConfidence(bounty, skillsMatch) {
     ...
     */
    calculateOverallScore(roi, skillsMatch, urgency, confidence, difficulty) {
        ...
        const score = (normalizedRoi * 0.4 + // 40% weight on ROI
     ...
     */
    generateReasoning(bounty, difficulty, skillsMatch, roi, urgency) {
        const reasoning = [];
     ...
     */
    getDifficultyReason(bounty, difficulty) {
     ...
     */
    async getTopBounties(bounties, limit = 5) {
     ...
     */
    async updateSkillsFromExperience(bounty, success) {
        if (!success)
            return; // Only learn from successful completions
