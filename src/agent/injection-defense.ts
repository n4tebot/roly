 ...
 */
export function analyzeInjection(input: string): InjectionAnalysis {
  ...
  for (const char of input) {
    ...
    if (code < 128) scripts.add('ascii');
    else if (code < 0x0250) scripts.add('latin');
    else if (code < 0x02B0) scripts.add('extended_latin');
  ...
  for (const term of financialTerms) {
    if (lowerInput.includes(term)) hasFinancial = true;
  }
  
  for (const term of urgencyTerms) {
    if (lowerInput.includes(term)) hasUrgency = true;
  }
 ...
 */
export function createSafeInputHandler(handler: (input: string) => Promise<any>) {
