...
export async function buildAgentContext(config: RolyConfig): Promise<AgentContext> {
  // Initialize clients
  ...
  try {
    ...
    const { threats, opportunities, goals } = await analyzeCurrentSituation(
      balanceInfo, 
      tier, 
      recentHistory,
      config
...
function determineSurvivalTier(usdcBalance: number, config: RolyConfig): SurvivalTier {
  const thresholds = config.survival.tiers;
  
  if (usdcBalance >= thresholds.normal) return SurvivalTier.NORMAL;
  if (usdcBalance >= thresholds.lowCompute) return SurvivalTier.LOW_COMPUTE;
  if (usdcBalance >= thresholds.critical) return SurvivalTier.CRITICAL;
  return SurvivalTier.DEAD;
}

function getCapabilities(tier: SurvivalTier) {
  switch (tier) {
...
async function getLastEarningTime(database: StateDatabase): Promise<Date | undefined> {
  try {
    // Look for recent turns involving income/earnings
    const recentTurns = await database.getRecentTurns(50);
    const earningTurn = recentTurns.find(turn => 
      turn.observation.toLowerCase().includes('received') ||
